...
(Path(__file__).resolve().parents[2] / "training-output").mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    with open(str(Path(__file__).resolve().parents[2] / "training-output" / "raw_features.json"), "w") as fh:
