...
WORK = str(Path(__file__).resolve().parents[2] / "training-output")
Path(WORK).mkdir(parents=True, exist_ok=True)
OUT = str(Path(__file__).resolve().parents[2] / "client" / "public" / "models" / "gesture-classifier")
...
if Path(f"{WORK}/probe_X.npy").exists():
