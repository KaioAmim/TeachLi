...
ROOT = str(Path(__file__).resolve().parents[2] / "client" / "public")
DATASET = str(Path(__file__).resolve().parents[2] / "datasets-raw" / "libras-alphabet")
...
def main():
    prev = json.load(open(str(Path(__file__).resolve().parents[2] / "training-output" / "raw_features.json")))
    ...
    json.dump(merged, open(str(Path(__file__).resolve().parents[2] / "training-output" / "features.json"), "w"))
